...
<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 text-white relative overflow-hidden">
      ...
      <div class="relative z-10 text-center max-w-4xl mx-auto transition-all duration-1000 ease-out" :class="{ 'opacity-0 translate-y-8': !isVisible, 'opacity-100 translate-y-0': isVisible }">
        <div class="mb-8">
          ...
          <div class="text-2xl md:text-3xl text-cyan-300 h-12 flex items-center justify-center">
            <span class="font-mono bg-gradient-to-r from-cyan-400 to-blue-400 bg-clip-text text-transparent">{{ displayText }}</span>
          </div>
        </div>
        ...
        <div class="grid grid-cols-3 gap-8 max-w-md mx-auto">
          <div class="text-center group">
            <div class="text-3xl font-bold bg-gradient-to-r from-cyan-400 to-blue-400 bg-clip-text text-transparent">{{ projectCount }}+</div>
            <div class="text-sm text-slate-400 group-hover:text-slate-300 transition-colors">Projects</div>
          </div>
          <div class="text-center group">
            <div class="text-3xl font-bold bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">{{ experienceYears }}+</div>
            <div class="text-sm text-slate-400 group-hover:text-slate-300 transition-colors">Years</div>
          </div>
          <div class="text-center group">
            <div class="text-3xl font-bold bg-gradient-to-r from-violet-400 to-purple-400 bg-clip-text text-transparent">{{ clientsServed }}+</div>
    ...
    <!-- About Section -->
    <section id="about" class="py-24 px-4 bg-slate-800/50 backdrop-blur-sm border-t border-slate-700/50">
        ...
        <div class="grid md:grid-cols-2 gap-12 items-center">
          <div class="space-y-6">
            ...
            <div class="flex flex-wrap gap-3">
              <span class="px-4 py-2 bg-gradient-to-r from-cyan-500/20 to-blue-500/20 border border-cyan-400/30 text-cyan-300 rounded-full text-sm font-medium backdrop-blur-sm hover:bg-cyan-500/30 transition-all">Creative Problem Solver</span>
              <span class="px-4 py-2 bg-gradient-to-r from-purple-500/20 to-pink-500/20 border border-purple-400/30 text-purple-300 rounded-full text-sm font-medium backdrop-blur-sm hover:bg-purple-500/30 transition-all">Team Player</span>
              <span class="px-4 py-2 bg-gradient-to-r from-violet-500/20 to-purple-500/20 border border-violet-400/30 text-violet-300 rounded-full text-sm font-medium backdrop-blur-sm hover:bg-violet-500/30 transition-all">Lifelong Learner</span>
            </div>
          </div>
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          >
            <div class="flex justify-between items-center mb-4">
              <span class="font-semibold text-lg text-slate-200 group-hover:text-white transition-colors">{{ skill.name }}</span>
